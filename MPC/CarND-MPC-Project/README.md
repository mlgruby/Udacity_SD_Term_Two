...
**[Working Video Link](https://www.youtube.com/watch?v=PvRUVPiP8-o)** 
...
The polynomial fitting is performed in the `polyfit` function in `main.cpp`, while the evaluation of the polynomial and it's slope are done in the functions `polyeval_cppad` and `polyeval_slope_cppad`, respectively in `MPC.h`. These functions were written to use the `CPPAD` namespace to facilitate automatic differentiation required by IPOPT. All computations are performed in the vehicle coordinate system. The transformation used is 
```
...
Here, I choose the second approach . The actuations are forced to remain at their previous values for the time of the latency. This is implemented in 
`MPC::Solve` like so. 
